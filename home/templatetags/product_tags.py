...
def product_filters(submenu, maincat, **afilters):
    return {'subcat': submenu, 'mcat': maincat, 'activefilters': afilters}


@register.inclusion_tag('product/include/filter_type.html')
def filter_type(maincat, menucat, istype):
    menu = Submenu.objects.get(name=menucat)
    types = ItemType.objects.filter(submenu=menu.id)
    print("FROM TAGS FILTER_TYPE: ",istype)
    return {
        'types': types,
        "submenu": menucat,
        'mcat': maincat,
...
def filter_price(menucat):
    ...
    prices = [
        {'name': 'sbdsfhdfsd3502398qwbia23',
            "range": '10,000 - 25,000', "quantity": gten},
...
def filter_discount(menucat):
    ...
    for discount in discounts:
        ...
        if discount.discountrange != 'none':
            obj.append(
                {'name': discount.name, 'range': discount.discountrange, 'quantity': count})
